...
export function backendRequest(path: string, init: RequestInit = {}): Promise<Response> { return fetch(`${BACKEND_API_URL}/${path.replace(/^\//, "")}`, { ...init, cache: "no-store", headers: { Accept: "application/json", "Content-Type": "application/json", ...init.headers }, signal: AbortSignal.timeout(10_000) }); }
