  ...
  return (
    <section className="mx-auto max-w-[1100px] px-5 py-8 sm:px-9 lg:px-12">
      ...
      {!loading && !loadError && data && <>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100 pt-8"><div><h2 className="text-2xl font-semibold text-[#0795ed]">Bank Details</h2><p className="mt-1 text-sm text-slate-500">Account numbers are encrypted. Verification remains pending until approved by a provider or administrator.</p></div>{data.can_manage && <button type="button" onClick={() => { setShowBankForm((current) => !current); setNotice(null); }} className="rounded bg-[#079ff0] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0788cf]">{showBankForm ? "Close form" : "+ Add Bank Account"}</button>}</div>

        {showBankForm && <form className="mt-6 grid gap-5 rounded-2xl border border-sky-200 bg-[#f6fbff] p-6 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void addBankAccount(); }}>
          <label className="text-sm font-medium text-[#0758a6]">Account Holder Name<input value={bankForm.account_holder_name} onChange={(event) => setBankForm((current) => ({ ...current, account_holder_name: event.target.value }))} className={fieldClass} /></label>
          <label className="text-sm font-medium text-[#0758a6]">Bank Name<input value={bankForm.bank_name} onChange={(event) => setBankForm((current) => ({ ...current, bank_name: event.target.value }))} className={fieldClass} /></label>
          <label className="text-sm font-medium text-[#0758a6]">Account Number<input type="password" inputMode="numeric" autoComplete="off" value={bankForm.account_number} onChange={(event) => setBankForm((current) => ({ ...current, account_number: event.target.value.replace(/\D/g, "").slice(0, 20) }))} className={fieldClass} /></label>
          <label className="text-sm font-medium text-[#0758a6]">Confirm Account Number<input inputMode="numeric" autoComplete="off" value={bankForm.account_number_confirmation} onChange={(event) => setBankForm((current) => ({ ...current, account_number_confirmation: event.target.value.replace(/\D/g, "").slice(0, 20) }))} className={fieldClass} /></label>
          <label className="text-sm font-medium text-[#0758a6]">IFSC Code<input value={bankForm.ifsc_code} maxLength={11} onChange={(event) => setBankForm((current) => ({ ...current, ifsc_code: event.target.value.toUpperCase().replace(/\s/g, "") }))} className={fieldClass} /></label>
          <label className="text-sm font-medium text-[#0758a6]">Branch Name<input value={bankForm.branch_name} onChange={(event) => setBankForm((current) => ({ ...current, branch_name: event.target.value }))} className={fieldClass} /></label>
          ...
          <label className="flex items-center gap-2 self-end pb-2 text-sm text-slate-700"><input type="checkbox" checked={bankForm.is_primary} onChange={(event) => setBankForm((current) => ({ ...current, is_primary: event.target.checked }))} className="size-4 accent-[#079ff0]" />Make this the primary account</label>
          <div className="flex gap-3 sm:col-span-2"><button type="submit" disabled={savingBank} className="rounded bg-[#0758a6] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#064a8a] disabled:opacity-60">{savingBank ? "Adding…" : "Add account"}</button><button type="button" disabled={savingBank} onClick={() => { setShowBankForm(false); setBankForm(emptyBankForm); }} className="rounded border border-slate-300 px-4 py-2.5 text-sm text-slate-600 hover:bg-white">Cancel</button></div>
        </form>}
